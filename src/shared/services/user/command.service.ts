...
export async function axiosGetCommandUserList(currentPage: number, limit: number) {
  try {
...
export async function axiosAddCommandUser(dataClient, cartItems) {
  try {
    const response = await axios.post(`${BASE_URL}/api/command/add`, {
      dataClient,
      cartItems
    })
    if (response.status >= 200 && response.status < 300) {
