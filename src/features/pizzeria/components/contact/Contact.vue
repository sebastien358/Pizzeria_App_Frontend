...
const onSubmit = handleSubmit(async (data, { resetForm }) => {
  try {
    const response = await contactStore.addContact(data)
    if (!response) {
      setErrorMessage("Erreur lors de l'envoi du message.")
      return
    }
    setSuccessMessage('Votre message a bien été envoyé', resetForm)
  } catch (e) {
    console.error(e)
...
const contactFormGsap = ref<HTMLElement | null>(null)
const contactInfoGsap = ref<HTMLElement | null>(null)

async function contactGsapAnimation() {
  const contact = contactFormGsap.value
  const info = contactInfoGsap.value
...
<template>
  <section class="contact-page">
    <div class="contact__container">
      <!-- FORMULAIRE -->
      <div class="contact__form" ref="contactFormGsap">
      ...
      <!-- INFOS + MAP -->
      <div class="contact__info" ref="contactInfoGsap">
        <h3>La pizzéria</h3>
        ...
        <div class="contact__badges">
          ...
          <span class="badge badge--yellow">🍕 Commande en ligne 7j/7</span>
        </div>
